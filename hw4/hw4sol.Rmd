...
The area under the curve is 0.5063. So the model built from this data set is not very accurate. 3 in 56 people will die for Cardiac Arrest in 30 days after being admitted to CCU. The 30-Day Mortality Rate of Cardiac Arrest Patients Admitted to CCU is about 5%.

## Neural network

We try 2 different methods of neural netwok. 

First we use Keras package to specify and train models.
...
Define a **sequential model** (a linear stack of layers) with 2 fully-connected hidden layers (129 and 50 neurons):
...
Then we try another method with `DMwR` package and nnet function:
...
Define the model:
After multiple try, we found maxit = 270 can achieve high accuracy around 72.33%
```{r}
