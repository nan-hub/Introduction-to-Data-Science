...
trainx<-train_x %>%
  mutate( age = age / max(age),
          ...
          admission_location = ifelse(admission_location == "TRANSFER FROM HOSP/EXTRAM",1,admission_location),
          admission_location = ifelse(admission_location == "CLINIC REFERRAL/PREMATURE",2,admission_location),
          admission_location = ifelse(admission_location == "EMERGENCY ROOM ADMIT",3,admission_location),
          admission_location = ifelse(admission_location == "PHYS REFERRAL/NORMAL DELI",4,admission_location),
...
model %>% 
  layer_dense(units = 129, activation = 'relu', input_shape = c(5)) %>% 
...
```{r}
model %>% predict_classes(testx) %>% head()
```
