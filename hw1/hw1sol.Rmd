...
```
The output is 46521. Ignoring the first line, there are 46520 unique patients in this data file.


-What are the possible values taken by each of the variable `INSURANCE`, `LANGUAGE`, `RELIGION`, `MARITAL_STATUS`, and `ETHNICITY`?
**Solution:**
```{bash echo=TRUE}
tail -58976 /home/203bdata/mimic-iii/ADMISSIONS.csv | awk -F ',' '{print $10}' | sort | uniq
tail -58976 /home/203bdata/mimic-iii/ADMISSIONS.csv | awk -F ',' '{print $11}' | sort | uniq
tail -58976 /home/203bdata/mimic-iii/ADMISSIONS.csv | awk -F ',' '{print $12}' | sort | uniq
tail -58976 /home/203bdata/mimic-iii/ADMISSIONS.csv | awk -F ',' '{print $13}' | sort | uniq
tail -58976 /home/203bdata/mimic-iii/ADMISSIONS.csv | awk -F ',' '{print $14}' | sort | uniq
```
...
**Solution:**
```{bash echo=TRUE}
awk '{for(i=1;i<=NF;i++){print $i}}' pride_and_prejudice.txt | grep "Jane"| wc -l 
awk '{for(i=1;i<=NF;i++){print $i}}' pride_and_prejudice.txt | grep "Elizabeth"| wc -l 
awk '{for(i=1;i<=NF;i++){print $i}}' pride_and_prejudice.txt | grep "Lydia"| wc -l 
awk '{for(i=1;i<=NF;i++){print $i}}' pride_and_prejudice.txt | grep "Darcy"| wc -l 
```
...
0. What's the difference between the following two commands?
    ...
    ```
**Solution:**
The < command directs 'hello, world' from one command to the file "test1.txt".
The << command appends 'hello, world' from one command to the file "test2.txt".
...
Use vi and save the file as `middle.sh`

Using `chmod` make the file executable by the owner,  
**Solutuion:**
```{bash}
chmod u+x pride_and_prejudice.txt
```
and run
...
report<-paste(mseSampAvg, msePrimeAvg, sep=",")
report
```
